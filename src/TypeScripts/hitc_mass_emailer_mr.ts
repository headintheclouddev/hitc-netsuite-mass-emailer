...
export function getInputData(): Object {
  // Find records where the status is "Not Started"
  const recordsToProcess: TaskEntity[] = [];
  const batchSize = runtime.getCurrentScript().getParameter({ name: 'custscript_hitc_mass_emailer_batch_size' }) as number;
  search.create({ type: 'customrecord_hitc_mass_email_task', filters: [['custrecord_hitc_mass_email_task_status', 'anyof', '1']] }).run().each((result) => {
    ...
    if (data.savedSearch && data.populateList != 'T') {
      ...
      if (data.searchField) {
        ...
        if (~data.searchField.indexOf('.')) join = data.searchField.substring(0, data.searchField.indexOf('.'));
      }
      const surplusRecipients: { type: string, id: string }[] = [];
      getAllResults(search.load({ id: data.savedSearch })).forEach((entityResult) => {
        ...
        if (columnName) {
          entityResult.columns.forEach((column) => {
            if (column.name == columnName && (!join || join == column.join)) entityId = entityResult.getValue(column) as string;
          });
        }
      ...
      if (batchSize) createTasksForSurplusRecipients(surplusRecipients, data);
    }
...
export const reduce: EntryPoints.MapReduce.reduce = (context) => {
  ...
  try {
    ...
    context.values.forEach((mapResult) => {
      if (mapResult == 'Processed') emailsSent++;
      else errors++;
    });
...
function createTasksForSurplusRecipients(surplusRecipients: { type: string, id: string }[], data: MASS_EMAIL_TASK): void {
  const batchSize = runtime.getCurrentScript().getParameter({ name: 'custscript_hitc_mass_emailer_batch_size' }) as number;
  data.savedSearch = '';
...
function getUnsubscribeURL(values: TaskEntity) {
...
function hasRecentBounce(recipient: Entity, campaignId: string): boolean {
...
function isUnsubscribed(recipient: Entity, campaignId: string): boolean {
  ...
  if (campaignId) {
    search.create({
      ...
      const subscribed = result.getValue('subscriptionstatus') as boolean;
...
function getAllResults(searchObj: search.Search): search.Result[] {
