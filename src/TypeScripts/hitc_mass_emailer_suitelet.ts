...
export function onRequest(context: EntryPoints.Suitelet.onRequestContext) { // [HITC-135] SSv2 conversion
  if (context.request.method == 'GET') {
    ...
    const relatedRecords: email.RelatedRecordTypes = {};
    const bcc    = getCCLines(context.request, 'bcc');
    const cc     = getCCLines(context.request, 'cc');
    const inputs = context.request.parameters as IEntryFields;
...
function drawForm(emailsSent: number): serverWidget.Form {
  const form = serverWidget.createForm({ title: 'Mass Emailer' });

  if (emailsSent) form.addPageInitMessage({ type: message.Type.CONFIRMATION, title: 'Success', message: `Sent ${emailsSent} emails.` });
  ...
  ['Contact', 'Employee', 'Partner', 'Customer'].forEach((entityTypeLabel) => {
    ...
    if (entityType == 'contact') sublist.addField({ id: 'custpage_contact_company', type: serverWidget.FieldType.TEXT, label: 'Company' })
...
function getCCLines(req: https.ServerRequest, ccOrBcc: string): string[] {
  const emails: string[] = [];
  ['contact', 'employee', 'partner', 'customer'].forEach((entityType) => {
    ...
    for (let line = 0; line < req.getLineCount({ group }); line++) {
      const emailAddress = req.getSublistValue({ group, name: `custpage_${entityType}_email`,      line }) as string;
      const doCopy       = req.getSublistValue({ group, name: `custpage_${entityType}_${ccOrBcc}`, line }) as string; // Yea, it is T/F for some reason
      // log.debug('getCCLines', `Sublist ${group} line ${line} do copy: ${doCopy}.`);
      if (doCopy == 'T') emails.push(emailAddress);
    }
  });
...
function getRecipientFromResult(result: search.Result, searchField: string): string {
  let recipient = '';
  if (searchField) { // We support parent.field joining
    ...
    if (~searchField.indexOf('.')) join = searchField.substring(0, searchField.indexOf('.'));
    result.columns.forEach((column) => {
      if (column.name == columnName && (!join || join == column.join)) recipient = result.getValue(column) as string;
    });
  } else {
    recipient = result.id;
  }
...
function mergeAndSendEmail(cc: string[], bcc: string[], relatedRecords: email.RelatedRecordTypes, attachments: file.File[], entityType: string, recipient: string, inputs: IEntryFields): boolean {
  const author     = Number(inputs.custpage_author);
  const recipients = [recipient];
  let   subject    = inputs.custpage_subject;
  let   body       = inputs.custpage_body;
  if (runtime.getCurrentScript().getRemainingUsage() < 20) return false;
...
function queueMapReduceScript(inputs: IEntryFields, cc: string[], bcc: string[], request: https.ServerRequest): string {
  const author       = Number(inputs.custpage_author);
...
interface IEntryFields {
  custpage_author:        string;
  custpage_subject:       string;
  custpage_body:          string;
  custpage_transaction:   string;
  custpage_print_mode:    string;
  custpage_template:      string;
  custpage_subscription:  string;
  custpage_saved_search:  string;
  custpage_search_type:   string;
  custpage_populate_list: string; // T or F
  custpage_search_field:  string;
}
